...
END $$;
        ...
        #     current_depth INT := 0;
        # BEGIN
        #     -- 1. Create a session-private workspace to prevent parallel collisions
        #     DROP TABLE IF EXISTS private_grid_workspace;
        #     CREATE TEMP TABLE private_grid_workspace AS 
        #     SELECT geom, id as coarse_id, 0 as depth 
        #     FROM {grid};

        #     CREATE INDEX ON private_grid_workspace USING GIST (geom);

        #     FOR current_depth IN 0..(max_depth - 1) LOOP
                
        #         -- Isolated table for the next generation calculation
        #         DROP TABLE IF EXISTS next_gen_step;
        #         CREATE TEMP TABLE next_gen_step (geom geometry, coarse_id int, depth int);

        #         -- 2. PART A: Keep "Light" squares from current depth
        #         INSERT INTO next_gen_step (geom, coarse_id, depth)
        #         SELECT geom, coarse_id, depth 
        #         FROM private_grid_workspace
        #         WHERE depth = current_depth
        #         AND (
        #             SELECT COALESCE(SUM(ST_NPoints(layer.geom)), 0)
        #             FROM {input} layer
        #             WHERE ST_Intersects(layer.geom, private_grid_workspace.geom)
        #         ) <= cutoff;

        #         -- 3. PART B: Split "Heavy" squares using Anti-Tilt interpolation
        #         INSERT INTO next_gen_step (geom, coarse_id, depth)
        #         SELECT 
        #             (ST_Dump(ST_Collect(ARRAY[
        #                 ST_MakePolygon(ST_MakeLine(ARRAY[p1, p12, pm, p41, p1])),
        #                 ST_MakePolygon(ST_MakeLine(ARRAY[p12, p2, p23, pm, p12])),
        #                 ST_MakePolygon(ST_MakeLine(ARRAY[pm, p23, p3, p34, pm])),
        #                 ST_MakePolygon(ST_MakeLine(ARRAY[p41, pm, p34, p4, p41]))
        ...
        #         FROM (
        #             SELECT 
        #                 geom, coarse_id, depth,
        #                 ST_PointN(ST_ExteriorRing(geom), 1) as p1,
        #                 ST_PointN(ST_ExteriorRing(geom), 2) as p2,
        #                 ST_PointN(ST_ExteriorRing(geom), 3) as p3,
        #                 ST_PointN(ST_ExteriorRing(geom), 4) as p4,
        #                 ST_LineInterpolatePoint(ST_MakeLine(ST_PointN(ST_ExteriorRing(geom), 1), ST_PointN(ST_ExteriorRing(geom), 2)), 0.5) as p12,
        #                 ST_LineInterpolatePoint(ST_MakeLine(ST_PointN(ST_ExteriorRing(geom), 2), ST_PointN(ST_ExteriorRing(geom), 3)), 0.5) as p23,
        #                 ST_LineInterpolatePoint(ST_MakeLine(ST_PointN(ST_ExteriorRing(geom), 3), ST_PointN(ST_ExteriorRing(geom), 4)), 0.5) as p34,
        #                 ST_LineInterpolatePoint(ST_MakeLine(ST_PointN(ST_ExteriorRing(geom), 4), ST_PointN(ST_ExteriorRing(geom), 1)), 0.5) as p41,
        #                 ST_Centroid(geom) as pm
        #             FROM private_grid_workspace
        #             WHERE depth = current_depth
        #             AND (
        #                 SELECT COALESCE(SUM(ST_NPoints(layer.geom)), 0)
        ...
        #         -- 4. PART C: Carry over rows already finalized at shallower depths
        #         INSERT INTO next_gen_step (geom, coarse_id, depth)
        #         SELECT geom, coarse_id, depth 
        #         FROM private_grid_workspace 
        #         WHERE depth < current_depth;
        ...
        #     -- 6. Output to the final persistent table defined by your app
        #     DROP TABLE IF EXISTS {refined_grid};
        #     CREATE TABLE {refined_grid} AS 
        #     SELECT row_number() OVER () as id, coarse_id, geom 
        #     FROM private_grid_workspace;
